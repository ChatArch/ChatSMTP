...
def test_tree_reports_registered_root_only_surface():
    ...
    assert result.output.splitlines().count("chatsmtp") == 1
...
def test_tree_brief_reports_the_same_root_only_nodes():
    ...
    assert brief.output.splitlines().count("chatsmtp") == 1
...
def test_tree_brief_omits_registered_command_signatures():
    ...
    def inspect(recipient: str) -> None:
        """Inspect a recipient; read-only text output."""

    full = CliRunner().invoke(sample, ["--tree"])
    brief = CliRunner().invoke(sample, ["--tree-brief"])

    assert full.exit_code == brief.exit_code == 0
    assert "inspect <RECIPIENT>" in full.output
    assert "inspect  # Inspect a recipient; read-only text output." in brief.output
    assert "<RECIPIENT>" not in brief.output
